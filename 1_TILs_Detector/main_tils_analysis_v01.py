...
def tumor_til_analysis(file_img,file_tumor,file_til,thr,mag):
    ...
    # if tumorb.shape!=LR.shape[0:2]:
    #     tumorb=transform.resize(tumorb,LR.shape[0:2],order=0) # order=0 nearest-neighbor

    # f=plt.figure()
    # plt.imshow(LR)
    # plt.contour(tumorb,[0.5],colors=['red'])
    # plt.savefig('../../../data/pan_cancer_tils/debug/'+file_img.split('/')[-1]+'.png')
    # f.clear()
    # plt.close()
    ...
    im.save('../../../data/pan_cancer_tils/debug/' + file_img.split('/')[-2]+'_'+file_img.split('/')[-1] + '.png')
...
if __name__=='__main__':
    ...
    tumorPath = ['../../../data/kang_colon_data/predictions_tumor/dl_model_v01/181119/',
