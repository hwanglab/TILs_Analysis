## purpose: perform univariate variable analysis, e.g., tils densities -> patient survivials
...
if (univ_analysis1==TRUE)
{
  for (nn in c("feat0","feat1","feat2","feat3","feat4")) # 5 features
  {
    ...
    pid2<-pid[-c(ind_na)]
    tt<-quantile(feat_v,0.5) # use median value to divide into high vs low
    plabel<-(feat_v>tt[1])
...
## 2) use univariate coxph function to perform univarate analysis
